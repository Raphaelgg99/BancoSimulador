...
public class JWTCreator {
    ...
    public static String create(String prefix, String key,JWTObject jwtObject) {
        String token = Jwts.builder().setSubject(jwtObject.getSubject()).setIssuedAt(jwtObject.getIssuedAt()).setExpiration(jwtObject.getExpiration())
                .claim(ROLES_AUTHORITIES, checkRoles(converter(jwtObject.getRoles()))).signWith(SignatureAlgorithm.HS512, key).compact();
        return prefix + " " + token;
    }
    ...
    public static JWTObject create(String token, String prefix, String key)
        ...
        object.setRoles(((Set) claims.get(ROLES_AUTHORITIES)));
        return object;
